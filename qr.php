...
// QR Code Generator for all tables
// Uses QRServer.com free API - no install needed
...
.qr-card{background:#fff;border-radius:16px;padding:20px;text-align:center;box-shadow:0 4px 16px rgba(15,118,110,.1);border:2px solid #d1faf5;page-break-inside:avoid}
.qr-card:hover{box-shadow:0 8px 24px rgba(15,118,110,.2);transform:translateY(-2px);transition:all .2s}
...
.qr-table-name{font-size:1.1rem;font-weight:700;margin-top:10px;color:#0f172a}
.qr-url{font-size:.65rem;color:#94a3b8;margin-top:4px;word-break:break-all}
.brand{margin-top:12px;font-size:.75rem;color:#0f766e;font-weight:600}
@media print{
  .page-header .controls{display:none}
  .page-header .btn-print{display:none}
  body{background:#fff}
  .qr-grid{padding:10px;gap:10px}
...
<div class="page-header">
  <div>
    ...
    <div class="page-sub">各テーブルのQRコードを印刷して設置してください</div>
  </div>
  <div class="controls">
...
?>
  <div class="qr-card">
    ...
    <img class="qr-img" src="<?php echo htmlspecialchars($qrApiUrl); ?>" alt="Table <?php echo $t; ?> QR" loading="lazy">
