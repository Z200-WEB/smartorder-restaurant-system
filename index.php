...
.cart-count{background:#fff;color:var(--mc-red,#e63946);border-radius:50%;width:18px;height:18px;font-size:.7rem;font-weight:700;display:inline-flex;align-items:center;justify-content:center}
...
.cat-btn:hover{border-color:var(--mc-red,#e63946);color:var(--mc-red,#e63946)}
.cat-btn.active{background:var(--mc-red,#e63946);border-color:var(--mc-red,#e63946);color:#fff;font-weight:600}
...
.item-add-btn{width:100%;padding:7px;background:var(--mc-red,#e63946);color:#fff;border:none;font-size:.8rem;font-weight:600;cursor:pointer;transition:background .2s}
...
.cart-header{background:linear-gradient(135deg,var(--mc-red,#e63946),#c1121f);color:#fff;padding:14px 16px;font-weight:700;font-size:1rem}
...
.qty-btn:hover{background:var(--mc-red,#e63946);color:#fff;border-color:var(--mc-red,#e63946)}
...
.cart-total-price{color:var(--mc-red,#e63946);font-size:1.1rem}
.btn-checkout{width:100%;padding:12px;background:var(--mc-red,#e63946);color:#fff;border:none;border-radius:8px;font-size:.95rem;font-weight:700;cursor:pointer;transition:background .2s}
.btn-checkout:hover{background:#c1121f}
.btn-checkout:disabled{background:#ccc;cursor:not-allowed}
.float-cart{display:none;position:fixed;bottom:20px;right:20px;background:var(--mc-red,#e63946);color:#fff;border:none;border-radius:999px;padding:12px 20px;font-size:.9rem;font-weight:700;cursor:pointer;box-shadow:0 4px 20px rgba(230,57,70,.4);z-index:200;align-items:center;gap:8px}
...
.modal-qty-btn:hover{background:var(--mc-red,#e63946);color:#fff}
...
.btn-order{flex:2;padding:10px;background:var(--mc-red,#e63946);color:#fff;border:none;border-radius:8px;font-size:.9rem;font-weight:700;cursor:pointer}
...
.btn-confirm-ok{flex:1;padding:10px;background:var(--mc-red,#e63946);color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600}
...
.spinner{width:40px;height:40px;border:4px solid #eee;border-top-color:var(--mc-red,#e63946);border-radius:50%;animation:spin .8s linear infinite}
...
<header class="app-header">
  <div class="header-inner">
    ...
    <div style="display:flex;gap:8px">
      <button class="btn-header" onclick="refreshStatus()">Status</button>
...
<nav class="category-nav">
  <div class="category-inner">
    ...
    <button class="cat-btn" onclick="filterCategory('<?php echo (int)$cat['id']; ?>',this)"><?php echo htmlspecialchars($cat['name']); ?></button>
    <?php endforeach; ?>
  </div>
</nav>
...
async function executeOrder(){
  ...
  try{
    ...
    const res=await fetch('api/addOrder.php',{method:'POST',body:fd});
    if(res.ok){showToast(selectedItemName+' ordered','success');await refreshCart();}
...
async function executeCheckout(){
  ...
  try{
    ...
    if(res.ok){showToast('Order confirmed!','success');await refreshCart();}
    else throw new Error('failed');
...
async function refreshCart(){
  try{
    const res=await fetch('api/getCart.php?tableNo='+TABLE_NO);
    if(res.ok){
      const data=await res.json();
