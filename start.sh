                                                ...
                                                if [ "$MYSQL_UP" -eq 0 ]; then
                                                    echo "ERROR: MariaDB did not start in time"
                                                        ...
                                                        mysql -u root --socket=/var/run/mysqld/mysqld.sock -e \
                                                            ...
                                                            TABLE_COUNT=$(mysql -u root --socket=/var/run/mysqld/mysqld.sock \
                                                                -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='practice';" \
                                                                    -sN 2>/dev/null || echo "0")

                                                                    if [ "$TABLE_COUNT" -eq "0" ] 2>/dev/null || [ -z "$TABLE_COUNT" ]; then
                                                                        echo "Importing database schema and seed data..."
                                                                            mysql -u root --socket=/var/run/mysqld/mysqld.sock practice \
                                                                                    < /var/www/html/database_utf8.sql
